...
class CSPResNet(nn.Module):
    ...
    def forward(self, x, output_layers=None):
        ...
        参数:
            x: 输入图像张量
            output_layers: 指定要返回的特征层，如果为None则使用self.return_idx
        """
        if output_layers is None:
            output_layers = self.return_idx
        
        # 如果output_layers是字符串列表，转换为整数列表
        if isinstance(output_layers, (list, tuple)) and len(output_layers) > 0:
            if isinstance(output_layers[0], str):
...
if __name__ == '__main__':
    ...
    m = CSPResNet("s", return_idx=[0, 1, 2, 3])
    outputs = m(data, ["2"])
